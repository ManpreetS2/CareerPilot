...
class TargetPreference(Base):
    ...
    opportunity_preference: Mapped[str | None] = mapped_column(String(32), nullable=True)
    experience_levels: Mapped[list] = mapped_column(JSON, default=list)
    skill_preferences: Mapped[list] = mapped_column(JSON, default=list)
...
class MatchScoreRecord(Base):
    ...
    match_tier: Mapped[str | None] = mapped_column(String(32), nullable=True)
    apply_recommendation: Mapped[str | None] = mapped_column(String(32), nullable=True)
    ...
    gap_reasons: Mapped[list] = mapped_column(JSON, default=list)
    watchouts: Mapped[list] = mapped_column(JSON, default=list)
    covered_responsibilities: Mapped[list] = mapped_column(JSON, default=list)
    partial_responsibilities: Mapped[list] = mapped_column(JSON, default=list)
    uncovered_responsibilities: Mapped[list] = mapped_column(JSON, default=list)
    created_at: Mapped[datetime] = mapped_column(
        DateTime, default=lambda: datetime.now(timezone.utc)
    )
...
class SavedSearchRecord(Base):
    ...
    employment_type: Mapped[list] = mapped_column(MutableList.as_mutable(JSON), default=list)
    work_mode: Mapped[list] = mapped_column(MutableList.as_mutable(JSON), default=list)
