...
def _preference_record_to_schema(record: TargetPreference) -> TargetPreferences:
    return TargetPreferences(
        ...
        work_mode_preferences=list(getattr(record, "work_mode_preferences", None) or []),
        relocation_willingness=getattr(record, "relocation_willingness", None),
        field_of_study=getattr(record, "field_of_study", None),
        industry_preferences=list(getattr(record, "industry_preferences", None) or []),
        opportunity_preference=getattr(record, "opportunity_preference", None),
        experience_levels=list(getattr(record, "experience_levels", None) or []),
        skill_preferences=list(getattr(record, "skill_preferences", None) or []),
