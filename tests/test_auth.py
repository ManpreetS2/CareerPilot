...
def test_create_session_then_resolve_by_token(isolated_session) -> None:
    user = auth_service.signup(isolated_session, "jordan@example.com", "a-real-password")
    token = auth_service.create_session(isolated_session, user)
    resolved = auth_service.get_user_by_token(isolated_session, token)
...
def test_get_user_by_token_returns_none_for_expired_session(isolated_session) -> None:
    ...
    expired = UserSession(
        token=hash_session_token("expired-token"),
        user_id=user.id,
...
def test_invalidate_session_deletes_it(isolated_session) -> None:
    user = auth_service.signup(isolated_session, "jordan@example.com", "a-real-password")
    token = auth_service.create_session(isolated_session, user)
    auth_service.invalidate_session(isolated_session, token)
    assert auth_service.get_user_by_token(isolated_session, token) is None
