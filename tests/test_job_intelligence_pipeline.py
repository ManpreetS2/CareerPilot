...
def test_manual_qa_gate_selects_ollama_first_when_configured(
    ...
    """Regression (defect #7): the real-description QA script used to hard-
    require a Gemini API key and a 'flash' model name, so it refused to run
    even when LLM_PROVIDER_ORDER put a fully configured Ollama first. The
    gate must follow LLM_PROVIDER_ORDER instead of hardcoding Gemini."""
